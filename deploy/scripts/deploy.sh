...
# 이전 이미지 태그 저장 (롤백용)
PREV_IMAGE=$(docker images bukery-01_tower --format "{{.ID}}" | head -1 || true)
...
for i in $(seq 1 12); do
  if curl -sf http://localhost:3000/webhook/health > /dev/null; then
    ...
    if [ -n "$PREV_IMAGE" ]; then
      CURR_IMAGE=$(docker images bukery-01_tower --format "{{.ID}}" | head -1)
...
if [ -n "$PREV_IMAGE" ]; then
  docker tag "$PREV_IMAGE" bukery-01_tower:latest
  docker compose -f deploy/docker-compose.yml up -d tower openclo
